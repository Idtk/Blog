...
**滑动是Android开发中非常重要的UI效果，本文将对滑动的使用以及原理进行介绍。**
...
不过scrollTo与ScrollBy的滑动操作都是瞬时完成的，无法满足我们对滑动的需求。
...
## 三、回弹效果
从上面的效果图可以看出，我们已经实现了view的平滑滚动，滑动位置超过当前view的1/2时，松手之后变会自动滑出此item的View。可是如果想要在首位两端实现回弹效果，改如何做呢？其实只要修改onTouchEvent方法即可。
```Java
@Override
public boolean onTouchEvent(MotionEvent event) {
    float xTouch = event.getX();
